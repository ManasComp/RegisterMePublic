    ...
    IZipService zipService) : IInvoiceSenderService
{
                ...
                switch (paymentInfo.PaymentType)
                {
                    ...
                    case PaymentType.PayByBankTransfer:
                        ...
                        messageBuilder
                            ...
                            .AddTable([
                                new RowWithHeader { Header = "IBAN", Value = [exhibition.BankAccount] },
                                new RowWithHeader { Header = "Účet", Value = [exhibition.Iban] },
                                new RowWithHeader
                                {
    ...
    public async Task<Stream> GetInvoicesZip(int registrationToExhibitionId, string webUrl, string rootPath)
    {
        HashSet<Invoice> paths =
            await invoiceCreator.CreateRegistrationToExhibitionInvoice(registrationToExhibitionId, webUrl, rootPath);
