...
public abstract class AbstractWorkflow : BaseEntity
{
    ...
    protected AbstractWorkflow(Workflow w)
    {
        ...
        GlobalParams = w.GlobalParams?.Select(x => new MyScopedParam(x)) ?? new List<MyScopedParam>();
        Rules = w.Rules?.Select(x => new MyRule(x)).ToList() ?? [];
    }
    ...
    /// </summary>
    public virtual IEnumerable<MyScopedParam> GlobalParams { get; set; } = null!;

    /// <summary>
    ///     List of rules.
    /// </summary>
    public virtual List<MyRule> Rules { get; set; } = null!;
