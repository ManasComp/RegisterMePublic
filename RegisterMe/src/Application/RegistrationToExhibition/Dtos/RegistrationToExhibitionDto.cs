...
public record RegistrationToExhibitionDto : CreateRegistrationToExhibitionDto
{
    ...
    public OrderStatus OrderStatus()
    {
        if (PaymentInfo != null)
        {
            if (PaymentInfo.SessionId != null && PaymentInfo.PaymentIntentId == null)
            {
                return Domain.Enums.OrderStatus.PaymentInProgress;
            }

            if (PaymentInfo.SessionId != null && PaymentInfo.PaymentIntentId != null)
            {
                return Domain.Enums.OrderStatus.PaymentCompleted;
            }
        }
