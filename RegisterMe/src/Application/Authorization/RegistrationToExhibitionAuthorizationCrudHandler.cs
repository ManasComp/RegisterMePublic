    ...
    Authorization<AuthorizeRegistrationToExhibitionId>(serviceScopeFactory, methods)
{
        ...
        OperationAuthorizationRequirement requirement, AuthorizeRegistrationToExhibitionId resource)
    {
        ...
        bool isOrganizationAdmin = applicationDbContext.RegistrationToExhibitions
        ...
        OperationAuthorizationRequirement requirement, AuthorizeRegistrationToExhibitionId resource)
    {
        ...
        string userId = AuthorizationHelperMethods.GetUserIdPub(context);
        bool isUser = applicationDbContext.RegistrationToExhibitions
        ...
        OperationAuthorizationRequirement requirement, AuthorizeRegistrationToExhibitionId resource)
    {
        ...
        bool registration =
            await applicationDbContext.RegistrationToExhibitions.AnyAsync(x => x.Id == registrationToExhibitionId);
        ...
        bool paymentRequested = await applicationDbContext.RegistrationToExhibitions
            .Where(x => x.Id == registrationToExhibitionId)
            .Include(x => x.PaymentInfo)
            .Select(x => x.PaymentInfo != null)
            .FirstOrDefaultAsync();
        ...
        AuthorizeRegistrationToExhibitionId resource)
    {
        ...
        Domain.Entities.RegistrationToExhibition? registration =
            await applicationDbContext.RegistrationToExhibitions.Where(x => x.Id == registrationToExhibitionId)
        ...
        string userId = AuthorizationHelperMethods.GetUserIdPub(context);
        if (registration.PaymentInfo == null)
        {
            bool isUser = applicationDbContext.RegistrationToExhibitions
                .Any(x => x.Exhibitor.AspNetUserId == userId && x.Id == registrationToExhibitionId);
