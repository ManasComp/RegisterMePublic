...
public class DeleteCatRegistrationAuthorizationTest(DatabaseTypes databaseType) : BaseTestFixture(databaseType)
{
    ...
    public async Task ShouldDeleteRegistration(RunAsSpecificUser runAsSpecificUser)
    {
        // Arrange
    ...
    public async Task ShouldFailDeleteCatRegistration(RunAsSpecificUser runAsSpecificUser)
    {
        // Arrange
        string ondrejId = await RunAsOndrejAsync();
        int organization1 = (await SendAsync(new CreateOrganizationCommand
        ...
        // Assert
        await deleteCatRegistration.Should().ThrowAsync<ForbiddenAccessException>();
    }
}
