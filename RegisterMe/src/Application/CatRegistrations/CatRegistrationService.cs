    ...
    : ICatRegistrationService
{
    ...
    {
        Result validationResult = await Validations(createCatRegistrationDto, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        CatRegistration? catRegistration =
            await appContext.CatRegistrations.FindAsync([catRegistrationId],
                cancellationToken);
        ...
        if (registrationToExhibition.PaymentInfo != null && !isExhibitionOrganizer)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Result validationResult = await Validations(createCatRegistration
            , cancellationToken);
        if (validationResult.IsFailure)
        {
            return Result.Failure<int>(validationResult.Error);
        ...
        if (catRegistrationEntity.RegistrationToExhibition.PaymentInfo != null && !isExhibitionOrganizer)
        {
    ...
    private Task<List<CatModelP>> GetUserCatsOrLittersNotInExhibitionBase<T>(
        ...
        where T : class
    {
        int exhibitorId = appContext.RegistrationToExhibitions
            .Where(x => x.Id == registrationToExhibitionId)
            .Select(x => x.ExhibitorId)
            .FirstOrDefault();
        IEnumerable<CatRegistration> catRegistrations = appContext.RegistrationToExhibitions
        ...
        HashSet<CatRegistration?> allRegistrationsExceptCurrent = catRegistrations
            .Where(x => x.RegistrationToExhibitionId != registrationToExhibitionId)
            .GroupBy(selector)
            .Select(g => g.MinBy(x => x.LastModified))
            .ToHashSet();

        HashSet<CatRegistration> currentRegistrations = catRegistrations
            .Where(x => x.RegistrationToExhibitionId == registrationToExhibitionId)
            .ToHashSet();

        List<CatModelP> data = allRegistrationsExceptCurrent
            .Where(x => !currentRegistrations.Any(y => comparer(x ?? throw new ArgumentNullException(nameof(x)),
    ...
    private MultiCurrencyPrice ComputePrice(HashSet<Record> records, List<Price> prices)
    {
        ...
        foreach (Record record in records)
        {
            ...
            foreach (Price? priceRecord in list)
            {
                ...
                possiblePricesKc.Add(priceRecord.PriceKc);
                possiblePricesEur.Add(priceRecord.PriceEur);
            }
