    ...
    TimeProvider timeProvider) : IRegistrationToExhibitionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        bool registrationsToExhibition = await appContext.RegistrationToExhibitions
        ...
        appContext.RegistrationToExhibitions.Add(registrationToExhibition);
        ...
        CancellationToken cancellationToken = default)
    {
        Domain.Entities.RegistrationToExhibition? catRegistration =
            await appContext.RegistrationToExhibitions
        ...
        await appContext.SaveChangesAsync(cancellationToken);
        appContext.RegistrationToExhibitions.Remove(catRegistration);
        await appContext.SaveChangesAsync(cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            query = query.Where(x =>
                    x.Exhibition.RegistrationEnd.ToDateTime(new TimeOnly()) < actualTime.ToUniversalTime().AddDays(2))
        ...
        List<TypedEmail> emails = registrationsToExhibitions.Select(x => new TypedEmail(x.PersonRegistration.Email,
            Type.IsDeleted, x.ExhibitionId)).ToList();

        appContext.CatRegistrations.RemoveRange(registrationsToExhibitions.SelectMany(x => x.CatRegistrations));
        await appContext.SaveChangesAsync(cancellationToken);
        appContext.RegistrationToExhibitions.RemoveRange(registrationsToExhibitions);
        await appContext.SaveChangesAsync(cancellationToken);
    ...
    public async Task<List<TypedEmail>> GetTemporaryRegistrationsEmails(CancellationToken cancellationToken = default)
    {
        ...
        List<TypedEmail> query = await
            appContext.RegistrationToExhibitions
                .Include(x => x.PaymentInfo)
                .Include(x => x.Exhibition)
                .Where(x => x.Exhibition.RegistrationEnd.ToDateTime(new TimeOnly()) <
                            actualTime.ToUniversalTime().AddDays(2))
                .Where(x => x.PaymentInfo == null)
    ...
    public async Task<RegistrationToExhibitionDto> GetRegistrationToExhibitionById(int registrationToExhibitionId,
        CancellationToken cancellationToken = default)
    {
        RegistrationToExhibitionDto? dto = await appContext.RegistrationToExhibitions
            .Where(x => x.Id == registrationToExhibitionId)
            .Include(x => x.PersonRegistration)
    ...
    {
        Domain.Entities.RegistrationToExhibition? registrationToExhibition = await appContext.RegistrationToExhibitions
    ...
    {
        var ems = await appContext.RegistrationToExhibitions
            .Where(x => x.Id == registrationToExhibitionId)
            .Include(x => x.CatRegistrations)
    ...
    public async Task<bool> HasActiveRegistrations(int exhibitionId, string userId, bool? paid,
        CancellationToken cancellationToken = default)
    {
        bool isRegistered = await appContext.RegistrationToExhibitions
            .Where(x => x.ExhibitionId == exhibitionId && x.Exhibitor.AspNetUserId == userId)
            .AnyAsync(
                x => paid == null || (x.PaymentInfo != null &&
                                      ((x.PaymentInfo.PaymentType != PaymentType.PayOnlineByCard &&
                                        x.PaymentInfo.PaymentRequestDate.HasValue == paid) ||
                                       (x.PaymentInfo.PaymentType == PaymentType.PayOnlineByCard &&
                                        x.PaymentInfo.PaymentCompletedDate.HasValue == paid))),
                cancellationToken);
    ...
    public async Task<bool> HasDrafts(int exhibitionId,
        CancellationToken cancellationToken = default)
    {
        bool hasDrafts = await appContext.RegistrationToExhibitions
            .Where(x => x.ExhibitionId == exhibitionId)
            .AnyAsync(x => x.PaymentInfo == null, cancellationToken);
        return hasDrafts;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Domain.Entities.RegistrationToExhibition registrationToExhibition = await
            appContext.RegistrationToExhibitions.Where(x => x.Id == registrationToExhibitionId)
                .Include(x => x.PaymentInfo)
        ...
        string paymentIntentId, CancellationToken cancellationToken = default)
    {
        ...
        Domain.Entities.RegistrationToExhibition? registrationToExhibition = appContext.RegistrationToExhibitions
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        registrationToExhibition.PaymentInfo.PaymentCompletedDate = timeProvider.GetLocalNow().ToUniversalTime();
        registrationToExhibition.PaymentInfo.Amount = price;
